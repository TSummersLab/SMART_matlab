function msfun_sphero_readsensor(block)
...
%% ---------------------------------------------------------
    ...
    function setup(block)
        ...
        block.NumDialogPrms     = 3; % spheroVar, T
...
%%
    function CheckPrms(block)        
        try
            ...
            validateattributes(block.DialogPrm(2).Data, {'char'}, {'nonempty'});  % Sensor Name
...
%%
    function Output(block)
        % fprintf('%s: Output\n', getfullname(block.BlockHandle));
        SensorValue=readSensor(mySphero,block.DialogPrm(2).Data);
        block.OutputPort(1).Data = double(SensorValue);
    end
