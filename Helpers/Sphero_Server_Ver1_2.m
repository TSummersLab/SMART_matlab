function [rob_pos, bboxesOrder, InitFrames, is_mine] = Sphero_Server_Ver1_2(ip_clients, gains, Adj, Dd, numRob, Sph, CameraParam, numRobLocal)
...
for i = 1:num_clients
    disp(strcat('Start client ',num2str(i)));
    fopen(servers{i});

    send_over_tcpip(servers{i}, s.bytes);
    send_over_tcpip(servers{i}, i);
    send_over_tcpip(servers{i}, num_clients);
    received = receive_over_tcpip(servers{i}); % check that data was received by client
