...
%
%
function SpheroState = FormationControl_FixScale_Ver3_2(iitr,itr,j, SpheroState, CameraParam)
...
%% Formation Control (fixed scale factor)
...
g = 1*(1/numRob)*(1/pi); % Gain
F = g * atan( (1/sat)* Adj.*(Dc-Dd) );
F = F + diag(-sum(F,2));
F2 = C2R(F);
% Normalize to have max value of 1
if any(abs(F2(:)) > 1)
    F2 = F2 ./ max(abs(F2));
end
...
dq = (A * q + F2 * q) .* vMax;            % Control velocity vectors
